...
export function BuddyAvatar() {
      ...
      try {
        if (enabled) {
          ...
          if (!prevEnabled.current) {
            await emit('buddy-event', { type: 'show' })
          }
        } else {
          // 关闭
  ...
  useEffect(() => {
    ...
    const syncConfig = async () => {
      try {
        await emit('buddy-event', { type: 'update' })
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const syncMood = async () => {
      try {
        await emit('buddy-event', { type: 'mood', data: { mood } })
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const syncSpeaking = async () => {
      try {
        if (isSpeaking) {
          await emit('buddy-event', { type: 'speaking' })
        }
      } catch (err) {
...
export async function sendBubbleToDesktop(text: string) {
  try {
    await emit('buddy-event', { type: 'message', data: { text } })
  } catch (err) {
